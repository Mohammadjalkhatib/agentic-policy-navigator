...
const ChatMessage = ({ message }) => {
    ...
    return (
        <div className={`flex items-start gap-4 ${message.role === 'user' ? '' : 'bg-white/80 dark:bg-gray-800/60 p-4 rounded-xl shadow-md border border-gray-100 dark:border-gray-700/50 backdrop-blur'}`}>
            {message.role === 'user' ? (
            ...
            <div className="flex-1">
                ...
                <div className="prose prose-sm dark:prose-invert max-w-none text-gray-700 dark:text-gray-300 relative group whitespace-pre-wrap leading-relaxed">
                    {message.text}
                    <button 
                        onClick={() => copyToClipboard(message.text)} 
                        className="absolute top-0 right-0 p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 opacity-0 group-hover:opacity-100 transition-opacity rounded-md hover:bg-gray-100 dark:hover:bg-gray-700/60"
                ...
                {message.sources && message.sources.length > 0 && (
                    <div className="mt-4">
                        <button
                            ...
                            className="flex items-center text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:underline underline-offset-2 decoration-dotted"
                        >
                            {isSourcesOpen ? <ChevronDown className="w-4 h-4 mr-1" /> : <ChevronRight className="w-4 h-4 mr-1" />}
                            Sources ({message.sources.length})
                        </button>
                        {isSourcesOpen && (
                            <div className="mt-2 pl-2 border-l-2 border-gray-200 dark:border-gray-700 space-y-2 bg-gray-50/60 dark:bg-gray-800/40 rounded-md p-2">
...
export default function App() {
    ...
    return (
        <div className={`${isDarkMode ? 'dark' : ''} flex h-screen bg-gradient-to-br from-slate-50 to-indigo-50 dark:from-gray-950 dark:to-slate-900 font-sans transition-colors duration-300`}>
            <ConfirmationModal config={modalConfig} onClose={() => setModalConfig(null)} />
            
            {/* Sidebar */}
            <aside className="w-1/3 max-w-sm p-6 bg-white/80 dark:bg-gray-900/70 backdrop-blur supports-[backdrop-filter]:bg-white/60 border-r border-gray-200 dark:border-gray-800 flex flex-col rounded-r-2xl shadow-lg">
                ...
                <div>
                    ...
                    <button
                        ...
                        disabled={isLoading || isRebuilding}
                        className="group w-full flex items-center justify-center p-3 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl text-gray-600 dark:text-gray-300 hover:bg-indigo-50/60 dark:hover:bg-indigo-900/20 hover:border-indigo-500 dark:hover:border-indigo-500 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm hover:shadow-md"
                    >
                        <Upload className="w-5 h-5 mr-2 transition-transform group-hover:-translate-y-0.5" />
                    ...
                    {indexedDocs.length > 0 && (
                        <div className="relative mb-2">
                            ...
                            <input
                                ...
                                className="w-full pl-10 pr-8 py-2 text-sm bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 hover:border-indigo-300 dark:hover:border-indigo-700 outline-none shadow-sm"
                            />
                            {docFilter && (
                                <button
                        ...
                        {filteredDocs.length > 0 ? (
                            <ul className="space-y-1">
                                {filteredDocs.map((doc, index) => (
                                    <li key={index} className="flex items-center text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 p-2 rounded transition-colors border border-transparent hover:border-indigo-200 dark:hover:border-indigo-700/50">
                    ...
                    <div className="space-y-1">
                        {exampleQuestions.slice(0, showAllExamples ? exampleQuestions.length : 3).map((q, idx) => (
                            <button 
                                ...
                                className="text-left w-full text-xs text-indigo-700 dark:text-indigo-300 bg-indigo-50/60 dark:bg-indigo-900/20 hover:bg-indigo-100 dark:hover:bg-indigo-900/30 px-3 py-1.5 rounded-full transition-colors"
                            >
                                "{q}"
                            </button>
                        ))}
            ...
            <div className="flex-1 flex flex-col h-screen bg-transparent">
                <header className="sticky top-0 z-10 bg-white/70 dark:bg-gray-900/60 backdrop-blur border-b border-gray-200 dark:border-gray-800 px-6 py-4 flex items-center justify-between flex-shrink-0">
                ...
                <footer className="p-6 md:p-8 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 flex-shrink-0">
                    {error && (
                        <div className="bg-red-100 dark:bg-red-900/30 border-l-4 border-red-500 text-red-700 dark:text-red-300 p-3 mb-4 rounded-xl text-sm shadow-sm" role="alert">
                            <div className="flex">
                    ...
                    <form onSubmit={handleAskQuestion} className="relative">
                        <textarea
                            ...
                            className="w-full p-4 pr-14 bg-gray-100 dark:bg-gray-800 border border-transparent rounded-2xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 hover:border-indigo-300 dark:hover:border-indigo-700 transition text-gray-800 dark:text-gray-200 placeholder-gray-500 dark:placeholder-gray-400 resize-none outline-none shadow-inner"
                        ...
                        <button
                            ...
                            className="absolute right-3 top-1/2 -translate-y-1/2 p-2 bg-gradient-to-r from-indigo-600 to-violet-600 text-white rounded-full hover:from-indigo-600 hover:to-indigo-700 disabled:from-indigo-300 disabled:to-indigo-300 dark:disabled:from-indigo-800 dark:disabled:to-indigo-800 disabled:cursor-not-allowed shadow-lg shadow-indigo-600/20 transition-all active:scale-95"
                            title="Send message"
                        >
